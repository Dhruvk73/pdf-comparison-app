...
def extract_product_data_with_llm(product_snippet_text: str, item_id_for_log="N/A", llm_model: str = "gpt-4o") -> dict:
    if not openai_client_instance: # Use the initialized instance
...
"unit_indicator": "c/u", "store_specific_terms": "*24 por tienda"
    ...
    try:
        ...
        logger.debug(f"ITEM_ID: {item_id_for_log} - extract_product_data_with_llm - Text LLM Raw Response Content: {response_content}") # Ensure this is logged
        ...
        # Log the data immediately after JSON parsing, before further modifications by your code
        ...
        logger.debug(f"ITEM_ID: {item_id_for_log} - extract_product_data_with_llm - LLM raw offer_price: {extracted_data.get('offer_price')}, LLM raw regular_price: {extracted_data.get('regular_price')}, LLM raw size_quantity_info: {extracted_data.get('size_quantity_info')}")

        extracted_data['offer_price'] = parse_price_string(extracted_data.get('offer_price'), item_id_for_log=f"{item_id_for_log}-llm_offer")
        # ... rest of the parsing ...
        logger.info(f"ITEM_ID: {item_id_for_log} - extract_product_data_with_llm - Successfully extracted and parsed data from Text LLM.")
        # The final `logger.debug` for parsed data is still useful if it's understood to be after your local parsing
        logger.debug(f"ITEM_ID: {item_id_for_log} - extract_product_data_with_llm - Final Internal Parsed LLM Data (after local parsing like parse_price_string): {json.dumps(extracted_data, indent=2)}")
        ...
        logger.info(f"ITEM_ID: {item_id_for_log} - extract_product_data_with_llm - Successfully extracted and parsed data from Text LLM.")
        logger.debug(f"ITEM_ID: {item_id_for_log} - extract_product_data_with_llm - Parsed LLM Data: {json.dumps(extracted_data, indent=2)}")
...
def re_extract_with_vision_llm(segment_image_bytes: BytesIO, item_id_for_log="N/A", original_item_name: str | None = None, llm_model: str = "gpt-4o") -> dict:
    ...
    "Return ONLY a JSON object with these fields. Use null for missing fields."
        ...
        response_content = chat_completion.choices[0].message.content
        response_content = chat_completion.choices[0].message.content
        # Log the raw response BEFORE any parsing or modification
        logger.debug(f"ITEM_ID: {item_id_for_log} - re_extract_with_vision_llm - Vision LLM Raw Response Content: {response_content}") # Ensure this is logged
        ...
        extracted_data = json.loads(response_content)
        # Log the data immediately after JSON parsing
        logger.debug(f"ITEM_ID: {item_id_for_log} - re_extract_with_vision_llm - Vision LLM Data (after json.loads): {json.dumps(extracted_data, indent=2)}")
        # You can also log specific fields if needed:
        logger.debug(f"ITEM_ID: {item_id_for_log} - re_extract_with_vision_llm - Vision LLM raw offer_price: {extracted_data.get('offer_price')}, Vision LLM raw size_quantity_info: {extracted_data.get('size_quantity_info')}")

        extracted_data["vision_llm_used"] = True # Add this flag
        logger.info(f"ITEM_ID: {item_id_for_log} - re_extract_with_vision_llm - Successfully extracted data from Vision LLM.")
        # logger.debug for final data is still good here
        return extracted_data
    except json.JSONDecodeError as je:
...
def collate_text_for_product_boxes(roboflow_boxes, textract_all_blocks, blocks_map,
    ...
    for i, box_pred in enumerate(roboflow_boxes):
        ...
        #      logger.debug(f"ITEM_ID: {item_id_for_log} - WordInBox {word_idx}: '{wb_debug.get('Text')}' Geom: {json.dumps(wb_debug.get('Geometry',{}).get('BoundingBox'))} Conf: {wb_debug.get('Confidence')}")
        ...
        for idx_w1, w1 in enumerate(words_in_rf_box):
            ...
            # Adjusted to catch 1 or 2 digits for dollars in geometric merge
            if not (re.fullmatch(r'[1-9]\d?', w1_text) and w1_geom_box): 
                continue
            ...
            for idx_w2 in range(idx_w1 + 1, len(words_in_rf_box)):
                ...
                if vertically_aligned and horizontally_close_and_ordered and height_compatible:
                    ...
                    logger.info(f"ITEM_ID: {item_id_for_log} - collate_text_for_product_boxes - Geometrically merged price candidate found: '{merged_geom_price_str}' from w1:'{w1_text}' (ID:{w1['Id']}) and w2:'{w2_text}' (ID:{w2['Id']})")
                    break 
        ...
        line_ids_in_box = set()
        for word_in_box in words_in_rf_box: # Use words to find parent lines
            # Traverse up to find LINE (simplified logic, Textract structure varies)
            # A more robust way might be to check if LINE center is in RF box.
            # For now, let's assume words_in_rf_box help define relevant lines.
            # This section might need more robust LINE selection based on RF box.
            # A simpler approach for now: Filter all LINE blocks by geometry.
            pass # Will filter all lines below.

        all_line_blocks_on_page = [block for block in textract_all_blocks if block['BlockType'] == 'LINE']
        for line_block in all_line_blocks_on_page:
        ...
        for line_block in lines_in_box_objects:
            ...
            elif not words_in_this_line_for_text and line_block.get('Text') and any(cid in used_word_ids_for_geom_price for rel in line_block.get('Relationships', []) if rel['Type'] == 'CHILD' for cid in rel.get('Ids',[])):
            ...
            elif not line_text and line_block.get('Text'): # Fallback for lines that Textract gives text for but no children not used
                logger.debug(f"ITEM_ID: {item_id_for_log} - LineCollation: Line '{line_block.get('Text')}' has no child words not used by geom_price, using raw line text if available and not fully geom-consumed.")
                # Check if all its words were used by geom_price
                all_words_used = True
                if 'Relationships' in line_block:
                    for relationship in line_block['Relationships']:
                        if relationship['Type'] == 'CHILD':
                            if not all(child_id in used_word_ids_for_geom_price for child_id in relationship['Ids'] if blocks_map.get(child_id, {}).get('BlockType') == 'WORD'):
                                ...
                                break
                    if not all_words_used : # Add if some words were not part of geom merge
                         ordered_lines_text_parts.append(line_block.get('Text','').strip())
                elif line_block.get('Text','').strip(): # No children, but has text
                     ordered_lines_text_parts.append(line_block.get('Text','').strip())
        ...
        # logger.debug(f"ITEM_ID: {item_id_for_log} - collate_text_for_product_boxes - Price candidates for this segment: {json.dumps(price_candidates_for_segment, indent=2)}")
...
def process_files_for_comparison(file1_bytes, file1_name, file2_bytes, file2_name):
    ...
    try:
        for file_idx_num_zero_based, (file_bytes_content, original_filename) in enumerate(
            ...
            # Use Werkzeug's secure_filename for consistent S3 key sanitization
            # but keep original_filename for reporting and internal logic where original name is preferred.
            ...
            if original_filename.lower().endswith(".pdf"):
                # Using tempfile for handling temporary PDF files
                fd, temp_pdf_path = tempfile.mkstemp(suffix=".pdf", prefix=f"{request_id}_")
                os.close(fd) # Close the file descriptor, we'll write to the path
                ...
                try:
                    ...
                    logger.info(f"{file_id_log_prefix} - Using POPPLER_BIN_PATH: {POPPLER_BIN_PATH}") # Add this line
                    ...
                    raise # Re-raise to be caught by Streamlit app
            ...
            for page_idx, page_image_pil in enumerate(page_pil_images_for_file):
                ...
                if not roboflow_model_object:
                    ...
                    # Decide if you want to proceed without Roboflow or raise an error
                    # For now, we'll try to continue, but collation will be empty
                    roboflow_preds = []
                else:
                    roboflow_preds = get_roboflow_predictions_sdk(page_image_pil, f"{s3_safe_filename_part}_p{page_idx}")
                
                if not roboflow_preds: # Check if None or empty
                    logger.warning(f"{page_id_log} - No Roboflow predictions or Roboflow failed. Skipping text collation for this page.")
                    # If Roboflow is essential, you might want to `continue` or handle differently
                
                # S3 Upload for Textract
                if not s3_client or not textract_client or not S3_BUCKET_NAME:
                    ...
                    # Decide flow: raise error, or try to proceed without Textract (which might not be feasible)
                    raise ConnectionError("S3/Textract not configured for backend processing.")


                img_byte_arr_s3 = BytesIO()
                page_image_pil.save(img_byte_arr_s3, format='JPEG', quality=90) # quality can be adjusted
                ...
                # Only collate if roboflow_preds were found
                collated_snippets = []
                if roboflow_preds:
            ...
            all_files_data_for_reprocessing_and_pils.append({
                ...
                "page_pils_list": page_pil_images_for_file, # List of PIL images for this file
        ...
        # Vision LLM re-processing (adapted from your Flask app)
        # backend_processor.py (snippet with fix for Issue #1)

# ... (previous parts of process_files_for_comparison) ...

  

        # backend_processor.py (snippet with fix for NameError and Issue #1)
...
# ... (previous parts of process_files_for_comparison) ...
        ...
        for file_idx, file_data_obj in enumerate(all_files_data_for_reprocessing_and_pils):
                ...
                for item_idx, item_to_evaluate_original in enumerate(file_data_obj["items"]):
                    ...
                    # --- START: CRUCIAL LOGIC BLOCK FOR send_to_vision and vision_reason ---
                    # This is where your original logic to decide if an item needs Vision LLM reprocessing must go.
                    # Ensure it uses 'item_being_processed' to fetch necessary data.
                    
                    send_to_vision = False  # Initialize to False for each item
                    vision_reason = "No specific trigger" # Initialize reason
                    ...
                    if item_being_processed.get("error_message"): # From initial text LLM
                    ...
                    # --- END: CRUCIAL LOGIC BLOCK FOR send_to_vision and vision_reason ---

                    # Now, the 'if send_to_vision...' can safely use these variables
                    if send_to_vision and item_being_processed.get("roboflow_box_coords_pixels_center_wh"):
                        ...
                        if page_idx_reproc < len(file_data_obj["page_pils_list"]):
                            ...
                            if segment_bytes:
                                ...
                                item_being_processed['reprocessed_by_vision_llm'] = True # Mark as attempted by vision

                                if "error_message" not in vision_llm_output:
                                    # ... (logging of raw vision output) ...
                                    ...
                                    # ... (logging after post_process_and_validate_item_data) ...
                                    
                                    # Update item_being_processed with fields from vision_item_processed if they are not None
                                    for key_update in ["offer_price", "regular_price", "product_brand", "product_name_core", 
                                                 "product_variant_description", "size_quantity_info", "unit_indicator", 
                                                 "store_specific_terms", "parsed_size_details", "size_quantity_info_normalized"]:
                                    ...
                                    item_being_processed['validation_flags'] = list(set(item_being_processed['validation_flags'])) # Remove potential duplicates
                            ...
                            item_being_processed['reprocessed_by_vision_llm'] = True
                    else: # Not sent to vision or missing Roboflow coordinates (if send_to_vision was true but coords were missing)
                        ...
                        # logger.debug(f"ITEM_ID: {item_id_for_log_vision} - Not sent to Vision LLM. Reason: {vision_reason if send_to_vision else 'No trigger met'}.")
...
# ... (rest of the function) ...
                ...
                # Log details for a specific item from File 2 to check its size fields
                for item_idx_debug, dbg_item in enumerate(final_product_items_file2):
                    if "RFBox0" in dbg_item.get("product_box_id", ""): # Example: check Ace Simply 84oz
                        logger.debug(f"ITEM_ID: {dbg_item.get('product_box_id')} (in final_product_items_file2) - "
                                    f"size_quantity_info: '{dbg_item.get('size_quantity_info')}', "
                                    f"size_quantity_info_normalized: '{dbg_item.get('size_quantity_info_normalized')}', "
                                    f"parsed_size_details: {dbg_item.get('parsed_size_details')}")
        # End of Vision LLM reprocessing loop

        logger.info(f"REQUEST_ID: {request_id} - Vision LLM re-processing stage complete.")
        logger.info(f"REQUEST_ID: {request_id} - Vision LLM re-processing stage complete.") # Existing log
        logger.info(f"REQUEST_ID: {request_id} - Final items for File 1 ({len(final_product_items_file1)}):")
        if final_product_items_file1: # Log a sample from File 1 for reference
            item_f1_sample = final_product_items_file1[0]
            logger.debug(f"REQUEST_ID: {request_id} - Sample File 1 Item ({item_f1_sample.get('product_box_id', 'N/A')}) PRE-COMPARE - "
                        f"SQI: '{item_f1_sample.get('size_quantity_info')}', "
                        f"SQI_Norm: '{item_f1_sample.get('size_quantity_info_normalized')}', "
                        f"ParsedDetails: {item_f1_sample.get('parsed_size_details')}")

        logger.info(f"REQUEST_ID: {request_id} - Final items for File 2 ({len(final_product_items_file2)}):")
        if not final_product_items_file2:
            logger.warning(f"REQUEST_ID: {request_id} - final_product_items_file2 IS EMPTY before comparison!")
        else:
            logger.debug(f"REQUEST_ID: {request_id} - DETAILED CHECK of final_product_items_file2 (first 5 items) PRE-COMPARE:")
            for i_debug, item_debug_f2 in enumerate(final_product_items_file2[:5]): # Log first 5 or a specific problematic ID
                item_id_dbg = item_debug_f2.get('product_box_id', f"UnknownID_F2_Item{i_debug}")
                logger.debug(f"ITEM_ID: {item_id_dbg} (in final_product_items_file2) PRE-COMPARE - "
                            f"PName: '{item_debug_f2.get('product_name_core', 'N/A')}', "
                            f"SQI: '{item_debug_f2.get('size_quantity_info', 'MISSING_SQI')}', "
                            f"SQI_Norm: '{item_debug_f2.get('size_quantity_info_normalized', 'MISSING_SQI_NORM')}', "
                            f"ParsedDetails: {item_debug_f2.get('parsed_size_details', 'MISSING_PARSED')}, "
                            f"Offer: {item_debug_f2.get('offer_price')}, Regular: {item_debug_f2.get('regular_price')}, "
                            f"ValFlags: {item_debug_f2.get('validation_flags', [])}, "
                            f"SuspectedHallucinationFlag: {item_debug_f2.get('suspected_size_hallucination', 'Not Set')}")

        # CRITICAL DEBUG: Check data integrity before comparison
        logger.info(f"REQUEST_ID: {request_id} - CRITICAL DEBUG: Data integrity check before comparison")
        logger.info(f"final_product_items_file1 length: {len(final_product_items_file1)}")
        logger.info(f"final_product_items_file2 length: {len(final_product_items_file2)}")

        # Check first few items from each file for critical fields
        for file_idx, items_list in enumerate([final_product_items_file1, final_product_items_file2]):
            file_label = f"File{file_idx + 1}"
            logger.info(f"CRITICAL DEBUG - {file_label} first 3 items critical fields:")
            
            for i, item in enumerate(items_list[:3]):
                item_id = item.get('product_box_id', f'Unknown_{file_label}_Item{i}')
                
                # Check the exact fields used in comparison
                brand = item.get('product_brand')
                name_core = item.get('product_name_core') 
                variant = item.get('product_variant_description')
                size_orig = item.get('size_quantity_info')
                size_norm = item.get('size_quantity_info_normalized')
                parsed_details = item.get('parsed_size_details')
                
                logger.info(f"ITEM_ID: {item_id}")
                logger.info(f"  product_brand: '{brand}' (type: {type(brand)})")
                logger.info(f"  product_name_core: '{name_core}' (type: {type(name_core)})")
                logger.info(f"  product_variant_description: '{variant}' (type: {type(variant)})")
                logger.info(f"  size_quantity_info: '{size_orig}' (type: {type(size_orig)})")
                logger.info(f"  size_quantity_info_normalized: '{size_norm}' (type: {type(size_norm)})")
                logger.info(f"  parsed_size_details: {parsed_details} (type: {type(parsed_details)})")
                
                # Check what the comparison function will actually use
                brand_lower = str(brand or "").lower().strip()
                name_lower = str(name_core or "").lower().strip()
                primary_text = f"{brand_lower} {name_lower}".strip()
                
                logger.info(f"  -> primary_text for comparison: '{primary_text}'")
                logger.info(f"  -> Is primary_text empty? {not primary_text}")
                logger.info("  " + "="*50)

        # Also check if the items exist but have been somehow corrupted
        logger.info(f"CRITICAL DEBUG - Sample raw item structures:")
        if final_product_items_file1:
            logger.info(f"File1 sample keys: {list(final_product_items_file1[0].keys())}")
        if final_product_items_file2:
            logger.info(f"File2 sample keys: {list(final_product_items_file2[0].keys())}")
        else:
            logger.error("CRITICAL: final_product_items_file2 is empty!")

        logger.info(f"REQUEST_ID: {request_id} - END CRITICAL DEBUG before comparison")
        ...
        # Prepare CSV data from the comparison report
        # The compare_product_items function already returns a list of dicts suitable for DataFrame
        if product_centric_comparison_report: # Ensure it's not empty
            report_df = pd.DataFrame(product_centric_comparison_report)
        else: # Handle empty comparison (e.g., no items in one or both files)
        ...
        final_response_dict = {
            ...
            "product_comparison_details": product_centric_comparison_report, # This is now list of dicts
        ...
        # Propagate the error so Streamlit can display a generic error message
        # Or return a specific error structure if Streamlit is designed to handle it:
        # return {
        #     "error": f"An internal server error occurred: {str(e_global)}",
        #     "product_comparison_details": [], # Ensure keys exist for Streamlit
        #     "report_csv_data": "Error generating report."
        # }
        raise # Simplest is to re-raise for Streamlit's default error handling
    finally:
        for temp_path in temp_pdf_paths_to_cleanup:
            if os.path.exists(temp_path):
                try:
                    os.remove(temp_path)
                    logger.info(f"REQUEST_ID: {request_id} - Cleaned up temp PDF: {temp_path}")
